...
class EC2Stack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    //Create EC2 role that will be assumed by EC2
    const roleEc2 = new aws_iam.Role(this, 'H0ck-Role-ForEc2', {
      assumedBy: new aws_iam.ServicePrincipal('ec2.amazonaws.com'),
    });

    roleEc2.addToPolicy(new aws_iam.PolicyStatement({
      effect: aws_iam.Effect.ALLOW,
      resources: ["*"],
      actions: ['lambda:InvokeFunction']
    }));

    //Find AWS account default VPC
    ...
    //Add policy to allow ssmm agent
    ...
    ssmaUserData.addCommands('export CDK_DEFAULT_REGION=' + process.env.CDK_DEFAULT_REGION)
