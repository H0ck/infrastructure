...
class LambdasStack extends Stack {
  ...
  constructor(scope, id, props) {
    ...
    fs.readdirSync(path.join(__dirname, "../frameworks/layers-built")).forEach(layerZip => {
      if (layerZip === 'layer-framework-scraping.zip') { return; }//TODO: Remove
      const layer = new aws_lambda.LayerVersion(this, layerZip, {
        layerVersionName: 'h0ck-infrastructure-lambda-layer-' + layerZip.replace(".zip", ""),
        code: aws_lambda.Code.fromAsset(path.join(__dirname, '../frameworks/layers-built', layerZip), {
        ...
        const handler = new aws_lambda.Function(this, lambdaName,
          {
            ...
            runtime: aws_lambda.Runtime.NODEJS_14_X,
            code: lambdaCode,
            layers: layers || ['layer-core-executor.zip'],
            handler: "/opt/nodejs/executor.handler",
            memorySize: lambdaDeployment.memory || 256,
            timeout: Duration.seconds(lambdaDeployment.timeout || 5),
            roles: [lambdaRole]
